...
private let SINCE_YEAR_PATTERN = "(^|\\s)depuis\\s+(?:l'\\s*)?(?:(?:ann\\u00e9e)|an)?\\s*([1-2]\\d{3})" +
    "(?!\\s*[-/\\.]\\s*\\d)(?=\\b|\\s|$)"
...
public class FRYearParser: Parser {
...
public class FRThisYearParser: Parser {
    ...
    override public func extract(text: String, ref: Date, match: NSTextCheckingResult, opt: [OptionType: Int]) -> ParsedResult? {
        ...
        result.tags[.frThisYearParser] = true
        return result
    }
}

public class FRLastYearParser: Parser {
    ...
    override public func extract(text: String, ref: Date, match: NSTextCheckingResult, opt: [OptionType: Int]) -> ParsedResult? {
        ...
        result.tags[.frLastYearParser] = true
        return result
    }
}

public class FRSinceYearParser: Parser {
    ...
    override public func extract(text: String, ref: Date, match: NSTextCheckingResult, opt: [OptionType: Int]) -> ParsedResult? {
        ...
        result.yearText = yearText
        result.tags[.frSinceYearParser] = true
        return result
    }
}

public class FRFromYearParser: Parser {
