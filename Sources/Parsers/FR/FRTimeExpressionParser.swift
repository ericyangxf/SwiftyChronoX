...
public class FRTimeExpressionParser: Parser {
    ...
    override public func extract(text: String, ref: Date, match: NSTextCheckingResult, opt: [OptionType: Int]) -> ParsedResult? {
        ...
        // contains an unambiguous time indicator (colon/h separator or AM/PM)
        if idx > 0 {
            ...
            if NSRegularExpression.isMatch(forPattern: "\\w", in: str) {
                ...
                let hasTimeSep = fullMatch.contains(":") || fullMatch.contains("：") || fullMatch.contains("h")
                let hasAmPm = match.isNotEmpty(atRangeIndex: amPmHourGroup)
                if !hasTimeSep && !hasAmPm {
                    return nil
                }
            }
        }
